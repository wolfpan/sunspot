    ...
    try {
        ...
        const response = await axios.post('https://open.bigmodel.cn/api/paas/v4/chat/completions', {
            ...
            messages: [
                {
                    role: "system", 
                    content: `你是一个空间天气数据转译引擎。必须严格执行以下规则：
1. 绝对禁止输出任何问候、解释、免责声明或格式化符号（如 Markdown 加粗或列表）。
2. 严禁使用“不仅如此”、“总体而言”、“事实上”等过渡词。
3. 严格遵循示例的句型结构。

输出模板：
观测现象：[客观描述黑子、流量、Kp指数状态，一句话总结环境]
环境影响：[描述极光、通信、卫星、微型电子设备受到的物理干扰程度]
综合太阳风暴指数：[仅输出1-10的整数]

示例输出：
观测现象：太阳黑子与射电流量处于中等活跃水平，地磁Kp指数极低，整体空间环境平静。
环境影响：极光不可见，短波通信受通量影响轻微，卫星轨道环境稳定，人体微型电子设备无物理干扰。
综合太阳风暴指数：2`
